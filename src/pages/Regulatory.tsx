...
const Regulatory = () => {
  ...
  const types = useMemo(() => ["All", ...Array.from(new Set(data.map(d => d.submission_type).filter(Boolean)))], [data]);
  ...
  const metrics = useMemo(() => {
    const activeSubmissions = filteredData.filter(d => 
      d.status?.toLowerCase().includes("active") || 
      d.status?.toLowerCase().includes("under review")
    ).length;

    const pendingReviews = filteredData.filter(d => 
      d.status?.toLowerCase().includes("pending") || 
      d.status?.toLowerCase().includes("in preparation")
    ).length;

    const actionItems = filteredData.filter(d => 
      d.key_issues && (
  ...
  const exportToCSV = () => {
    const headers = [
      "Timestamp", "Agency", "Region", "Submission ID", "Submission Type",
      "Compound Name", "Generic Name", "Therapeutic Area", "Indication",
      "Status", "Priority Designation", "Submission Date", "Target Decision Date",
      "Approval Date", "Review Cycle", "Key Issues", "Risk Level", "Impact",
      "Source", "Summary", "Last Updated By"
    ];
  ...
  const getStatusColor = (status: string) => {
    ...
    if (s.includes("rejected") || s.includes("withdrawn")) return "bg-destructive/20 text-destructive border-destructive/30";
      ...
      <GlassCard className="overflow-hidden">
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="p-8 text-center text-text-light-gray">No records found matching the selected filters.</div>
          ) : (
            <Table>
              <TableHeader>
